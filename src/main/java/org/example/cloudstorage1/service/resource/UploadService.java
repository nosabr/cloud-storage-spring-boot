...
public class UploadService {
    ...
    public List<FileNode> upload(String path, List<MultipartFile> files, User user) throws IOException {
        log.info("Uploading files to directory: {}", files);
        checkForConflict(path, files, user);
        log.info("NO conflicts");
        List<ParsedFile> parsedFiles = parseFiles(files);
        log.info("Parse file {}", parsedFiles);
        createFolderIfNotExist(path, parsedFiles, user);
        ...
        try {
            return addParsedFilesToDB(path, parsedFiles, user);
        } catch (Exception e) {
    ...
    private void checkForConflict(String path, List<MultipartFile> files, User user) {
    ...
    private void createFolderIfNotExist(String path, List<ParsedFile> parsedFiles, User user) {
        for (ParsedFile parsedFile : parsedFiles) {
            ...
            for(int i = 0; i < segments.size() - 1; i++) {
                ...
                try {
                    ...
                    log.info("Folder already exists, skip {}", parsedFile);
                }
            }
        }
    }
    ...
    private List<FileNode> addParsedFilesToDB(String path, List<ParsedFile> parsedFiles, User user) {
        List<FileNode> addedFileNodes = new ArrayList<>();
        for (ParsedFile parsedFile : parsedFiles) {
            FileNode fileNode = createFileNode(path, parsedFile, user);
            addedFileNodes.add(fileNodeRepository.save(fileNode));
        }
        return addedFileNodes;
    }

    private FileNode createFileNode(String path, ParsedFile parsedFile, User user) {
        Long lastParentId = getFileParentId(path, parsedFile, user);
        return FileNode.builder()
                .name(parsedFile.getFileName())
                .type(FileType.FILE)
                .parentId(lastParentId)
                .path(path + parsedFile.file().getOriginalFilename())
                .ownerId(user.getId())
    ...
    private Long getFileParentId(String path, ParsedFile parsedFile, User user) {
        if(parsedFile.path().isEmpty() && path.isEmpty()){
            return null;
        }
        String fullPath = path + parsedFile.path();
        FileNode newParentFileNode = fileNodeRepository.findByOwnerIdAndPath(user.getId(), fullPath)
                .orElseThrow(() -> new ResourceNotFoundException("Resource not found " + fullPath));
